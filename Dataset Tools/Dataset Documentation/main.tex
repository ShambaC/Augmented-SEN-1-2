\documentclass[12pt, a4paper, notitlepage, twocolumn]{article}

\usepackage{graphicx}
\usepackage{authblk}
\usepackage[left=1.5cm, right=1.5cm, top=2cm, bottom=2cm]{geometry}
\usepackage[
    pdfauthor={Shamba Chowdhury, Ankana Ghosh, Shreyashi Ghosh},
    pdftitle={Augmented Sentinel 1-2 Dataset}
]{hyperref}
\usepackage{amsmath}
\usepackage{listings}
\usepackage{color}
\usepackage{dblfloatfix}

\definecolor{dkgreen}{rgb}{0,0.6,0}
\definecolor{gray}{rgb}{0.5,0.5,0.5}
\definecolor{mauve}{rgb}{0.58,0,0.82}

\lstset{frame=tb,
  language=Java,
  aboveskip=3mm,
  belowskip=3mm,
  showstringspaces=false,
  columns=flexible,
  basicstyle={\small\ttfamily},
  numbers=none,
  numberstyle=\tiny\color{gray},
  keywordstyle=\color{blue},
  commentstyle=\color{dkgreen},
  stringstyle=\color{mauve},
  breaklines=true,
  breakatwhitespace=true,
  tabsize=4
}

\graphicspath{{images/}}

\title{\textbf{Augmented Sentinel 1-2 Dataset}}

\author[1]{Shamba Chowdhury}
\author[1]{Ankana Ghosh}
\author[1]{Shreyashi Ghosh}

\affil[1]{\small{Department of Computer Science and Engineering, University of Calcutta}}

\date{April 2025}

\begin{document}

\twocolumn[
    \begin{@twocolumnfalse}
        \maketitle

        \begin{abstract}
            Deep learning techniques are having a major impact on every technical fields now-a-days and gathering sufficient amounts of training data was challenging in the space of remote sensing. This problem was solved using the Sen 1-2 dataset. The dataset contains almost 300,000 pairs of SAR and optical images across multiple seasons and regions. We have extended the dataset with an additional 37,437 pair of images. Furthermore, the dataset has been augmented with temperature region information for all images. We believe that this data augmentation will help in the progression of various SAR image applications.
        \end{abstract}
    \end{@twocolumnfalse}
]

\section{Introduction}
\input{chapters/01_introduction.tex}

\section{The SEN1-2 Dataset}
\input{chapters/02_tumdataset.tex}

\section{The Dataset}
\input{chapters/03_dataset.tex}

\section{Example Applications}
\input{chapters/04_applications.tex}

\section{Advantages and Disadvantages of the Dataset}
\input{chapters/05_adv_disadv.tex}

\section{Summary and Conclusion}
\input{chapters/06_Summary.tex}

\bibliographystyle{unsrt}
\bibliography{references}

\end{document}